...
if ($res->num_rows > 0)
{
	while ($row = $res->fetch_assoc())
	{
        ...
        $html .= '<td><a class="action" href="?page=' . $_GET['page'] . '&action=delete&id=' . $row ['id'] . '" title="usuń" onclick="return confirm(\'Czy na pewno chcesz usunąć ten wpis?\');"><i class="fa fa-times text-danger"></i>usuń</a></td>';
        $html .= '<td><a class="action" href="?page=' . $_GET['page'] . '&action=edit&id=' . $row ['id'] . '" title="edytuj" <a><i class="fa fa-pencil text-danger"></i>edytuj</a></td>';
        $html .= '</tr>';
	}
    $html .= "</tbody>";
    $html .= "</table>";
